...
const Listnow = (props) => {
    var x=new Date(props.i1.timestamp).toString()
    console.log(x)
    return (
        ...
        <Card style={{ borderWidth: 0.7 ,backgroundColor:'#f8f8ff',marginTop:0.5}}>
           ...
           <Text >{props.i1.name}</Text>
           <Text >{props.i1.price}</Text>
           <Text >{props.i1.quantity}</Text>
           <Text >{props.i1.suggestions}</Text>
       </Card.Content>
   </Card>
...
const OrderDetails = ({route}) => {
    ...
    console.log(route.params.data.orderList)
    return (
        <View style={styles.container}>
     ...
     <Text style={{ fontSize: 15,textAlign: 'center', fontWeight: "600",color:'#fff' }}>{x}:-
      ...
      <View style={styles.itemLeft}>
            <FlatList
                data={route.params.data.orderList}
                renderItem={({ item, index }) => (<Listnow i1={item} />)}
